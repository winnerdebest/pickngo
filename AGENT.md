...
- **2026-09-24 Database Migrations (Alembic):** Configured Alembic (`alembic.ini`, `alembic/env.py`) for version-controlled database schema migrations across environments. `alembic/env.py` dynamically sources `settings.DATABASE_URL` from `.env` and wires `target_metadata = Base.metadata`. Added initial revision `7ca1dd6eca2a_initial_schema_customers_runners_tasks.py`.
